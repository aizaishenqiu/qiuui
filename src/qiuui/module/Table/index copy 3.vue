<template>
  <div class="qiu-table-container">
      ...
      <div v-for="(item, index) in toolbar" :key="index" :class="item.class">
        <div v-if="item.data.length > 1" class="qiu-btn-group">
          <span v-for="(se, ind) in item.data" :key="ind">
            <button
              :class="se.class"
              @click="se.fun(delfield(celldata), delfield(state.checkdata))"
            >
              <i :class="se.icon"></i>
              {{ se.text }}
            </button>
    ...
    <div v-if="page" id="qiu-table-page"></div>
  </div>
</template>
  ...
  setup(props) {
    ...
    function getpagedata() {
      ...
      qiuPage({
        ...
        elem: "qiu-table-page-" + props.tableid,
        skin: page.skin,
        first: page.first,
    ...
    function unialldata(a) {
      ...
      if (props.page) {
        return findindex(props.cols, state.Pagedata);
      } else {
    ...
    function editinput(e, key, index, type, tp) {
      ...
      if (type == "up") {
        ...
        //   let pn = tp.node;
        //   pn.style.display = "";
        //   pn.innerText = vn.value;
        //   pn.parentNode.removeChild(vn);
        //   celldata[tp.ind][key] = vn.value;
        // }
        console.log(tp, type);
        np = 9699;
      }
    ...
    function editleave(e, key, index, type, tp) {
      ...
      if (tp) {
        ...
        if (e.previousSibling) {
          ...
          e.previousSibling.innerText = e.value;
          e.parentNode.removeChild(e);
          // 修改源数据
          celldata[index][key] = e.value;
        }
      } else {
        console.log("false");
        e.previousSibling.style.display = "";
    ...
    function upfocus(key, index, id, oid) {
      ...
      if (index > 0 && index < celldata.length) {
        row = index;
      } else {
        row = 1;
      }
    ...
    function downfocus(key, index, id, oid) {
      ...
      if (index < celldata.length - 1 || index > 0) {
        row = index + 2;
      } else {
        row = 2;
      }
    ...
    function trodd() {
      ...
      for (let i = 1; i < trnum.length - 1; i++) {
        if (i % 2 !== 1) {
    ...
    function celltotal(key) {
      ...
      for (let i in celldata) {
        for (let keyd in celldata[i]) {
          if (keyd == key) {
            num.push(Number(celldata[i][key].replace(/,/g, "")));
          }
        }
      }
